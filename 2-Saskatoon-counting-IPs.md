...
- `awk` the first column from the access log file. This is the IP address.
- sort these entries
- use `uniq -c` to find the count of occurrences
- use `sort -nrk1` which sorts the above result numerically in the reverse order based on the first column which contains the count
- use `head -n1` to get the first entry [Ignore the additional head in the screenshot]
- this will have the count and the IP, so, `awk` the second column
- pipe it to a file as required